...
## ml_ds Classification Target & Churn-Coupling (Issue #347 — auditoría, de-churn diferido)

- Scope: `studentProfile == "ml_ds"` + family `clasificacion`. El esquema fijo M2 son 18 columnas SaaS/churn y la binaria `categoria` deriva su señal de `churn_rate`. El spine determinista #301 (`_enforce_business_classification_schema`, `_binary_target_column`, `_ensure_both_classes`) es **business-only** y NO corre para ml_ds (`_enforce_business_classification_schema` early-return si `profile != "business"`).
- Cadena determinista real en `schema_designer` (`graph.py` happy ~3760-3769, fallback ~3802-3809): `_align_ml_ds_classification_target` (ml_ds: renombra la binaria `categoria` al nombre del target del contrato CONSERVANDO su driver `churn_rate`; borra binarias duplicadas del mismo driver; corre ANTES del augment) → `_augment_schema_with_contract` (R2: un `classification_target` int `.isidentifier()` ausente se inyecta binario {0,1} con `_pick_numeric_driver`, que prefiere `churn_rate`; el `else` cae a `_default_column` int[0,100] `dependency=None`) → `_enforce_business_classification_schema` (**NOOP para ml_ds**).
- Realidad medida (corrida determinista de la auditoría): para un caso ml_ds **no-churn** (p.ej. `fraud_flag`) el target sale binario {0,1}, con ambas clases, calibrado a `target_event_rate` y CON señal (AUC CV ≈ 0.8, dentro de [0.55,0.99]) → **el caso COMPLETA**. PERO la señal proviene de `churn_rate` (no del dominio), las columnas churn permanecen, y las features de dominio del contrato son ruido (`dependency=None`). El defecto es de **coherencia pedagógica**, NO un fallo de gate.
- ⚠️ El cuerpo original de #347 ("target sin señal → AUC ~0.5 → FALLA el gate") está DESFASADO (describe el mundo pre-#348). **El gate AUC-fuera-de-rango es NO bloqueante**: `is_m3_quality_warning_blocking` (`m3_notebook_execution.py:495`) solo bloquea por `marker_missing/invalid` y `auc_missing` (no-skip); `m3_quality_auc_out_of_range` (línea 477) es warning → el caso completa. No re-perseguir esa premisa.
- **De-churn diferido** (alto riesgo: generador determinista + snapshots/hashes congelados; defecto = coherencia, no crash). Guardado por `backend/tests/test_issue347_mlds_target_signal_guard.py`: fija el invariante "target de contrato no-churn binario, ambas clases, con señal (AUC CV ≥ 0.55)" SIN acoplar a churn → sigue verde si se implementa el de-churn real (target ← driver de dominio), pero rojo si el path no-churn pierde la señal (la regresión que #347 temía; probado por revert: `dependency=None` → AUC 0.508).
- Coordinación para el de-churn real (cuando se haga): bundle con #346 (narrativa M2) y respetar líneas rojas — churn/retención byte-idéntico (gate `_is_retention_target_name`), cero cambio en business/`regresion`/`clustering`/`serie_temporal`, fila count 600. Patrón a reusar: un SIBLING `_enforce_mlds_classification_schema` (NO generalizar la función business — `test_enforce_noop_for_ml_ds` lo prohíbe).
...
- Guards en `test_m2_clasificacion_dispatch.py`: `test_eda_text_ml_ds_not_churn_hardcoded`, `test_eda_questions_classification_not_churn` (ambos perfiles), `test_eda_questions_classification_preserves_pedagogy`; los contratos de placeholders (18 EDA-texto / 7 EDA-preguntas) NO cambian.
- Follow-up (coordinado con #347): el prompt EDA aún usa el literal `categoria` mientras `_align_ml_ds_classification_target` renombra la columna del dataset al nombre del contrato (`fraud_flag`, etc.) → mismatch de NOMBRE de columna que el fallback "si `categoria` no existe…" papelea. Se difiere al de-churn del DATO (#347), donde la estrategia de nombrado puede cambiar. #346 cierra la incoherencia de NARRATIVA; #347 cerrará la del dato.
