...
class Settings(BaseSettings):
    ...
    # M1→M5 verdict coherence for ml_ds + clasificacion. When true (default), M1's recommended
    # strategic option (P3 `solucion_esperada`, A/B/C) is propagated to the M5 final memo so the case
    # does not contradict itself about the answer (the classification analog of the clustering #467
    # defect "M1→A/B while M4/M5→C"): a brace-free nudge makes P3 state its option parseably, the M5
    # memo prompt gets a hint to honor that option, and a deterministic reprompt-once-then-degrade
    # guard enforces it. "M1 leads" — M1 still PICKS the option; M4 is out of scope (its §4.5 is a
    # model-deploy verdict, not an A/B/C choice). Set CLASSIFICATION_VERDICT_COHERENCE=false to revert
    # byte-identically (no M1 hint, no M5 hint, no guard; instant env-only revert, no redeploy). The
    # gate is profile=="ml_ds" AND family=="clasificacion"; every other cohort is unaffected either way.
    classification_verdict_coherence: bool = True
