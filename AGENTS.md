...
- Follow-up: **#383** (literal `categoria` → `{target_column_name}` en M2 EDA, independiente). Edge **Caso A** (ml_ds+clf SIN `dataset_schema_required`): mantiene el template churn (sin dominio que inferir; sibling no-op) — aceptable, igual que antes.
...
- Follow-up: el prompt EDA aún usa el literal `categoria` mientras `_align_ml_ds_classification_target` renombra la columna del dataset al nombre del contrato (`fraud_flag`, etc.) → mismatch de NOMBRE de columna que el fallback "si `categoria` no existe…" papelea. El de-churn del DATO (#382) NO tocó ese literal (mantuvo `categoria` en `eda_text.py`); se difiere a **#383** (`{target_column_name}` threading). #346 cerró la incoherencia de NARRATIVA, #382 la del DATO (señal), #383 cerrará la del nombre de columna.
