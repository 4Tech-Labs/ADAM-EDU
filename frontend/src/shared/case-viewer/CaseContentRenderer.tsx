...
// (lr_only / rf_only / lr_rf_contrast) in notebooks/_shared.py and notebook.py — it is
// stable across all classification families and will not change without an AGENTS.md update.
...
export function injectNotebookRefs(html: string | null | undefined): string | null | undefined {
    ...
    for (const [headingId, sectionRef] of Object.entries(NOTEBOOK_HEADING_REFS)) {
        ...
        result = result.replace(
            re,
            `$1 data-nb-ref="${sectionRef}"$2$3 <code class="ml-1.5 text-xs font-mono font-semibold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded whitespace-nowrap" aria-label="Sección del notebook">&sect;&thinsp;${sectionRef}</code>$4`,
        );
    }
    return result;
}
