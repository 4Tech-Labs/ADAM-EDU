...
// Maps heading IDs (generated by renderMarkdownWithIds after NFD normalization) to the
// corresponding notebook section reference shown as a badge next to the heading.
// The single-model variants (lr_only / rf_only) number the cost-matrix section 3.0.5.6; the
// (form-hidden) contrast variant uses 3.0.5.7 because its extra pipeline cell shifts the
// numbering. Source of truth: TOC_MARKDOWN_CELL_BY_VARIANT in
// backend/src/case_generator/prompts/clasificacion/notebooks/_shared.py. The teacher form
// only emits single-model jobs today, so the single-model number is correct for every
// generatable case; if the contrast variant is re-enabled, this must become variant-aware.
// Keep in sync with the backend notebook source if the numbering changes again.
const NOTEBOOK_HEADING_REFS: Readonly<Record<string, string>> = {
    "seccion-como-leer-la-matriz-de-costos": "3.0.5.6",
};

/**
 * Appends a notebook section-reference badge (§ X.Y.Z) to specific M3 narrative headings.
 *
 * Must be called AFTER renderMarkdownWithIds — relies on the deterministic IDs it generates.
 * Called exclusively for m3Content; all other modules are unaffected.
 * Safe on null/undefined (passthrough). Idempotent via data-nb-ref attribute guard.
 */
export function injectNotebookRefs(html: string): string;
export function injectNotebookRefs(html: null): null;
export function injectNotebookRefs(html: undefined): undefined;
export function injectNotebookRefs(html: string | null): string | null;
// eslint-disable-next-line react-refresh/only-export-components
export function injectNotebookRefs(html: string | null | undefined): string | null | undefined {
    if (!html) return html;
    let result = html;
    for (const [headingId, sectionRef] of Object.entries(NOTEBOOK_HEADING_REFS)) {
        // Negative lookahead on data-nb-ref prevents double-injection on re-renders.
        const re = new RegExp(
            `(<h[23] id="${headingId}"(?![^>]*data-nb-ref)[^>]*)(>)(.*?)(</h[23]>)`,
            "gi",
        );
        result = result.replace(
            re,
            `$1 data-nb-ref="${sectionRef}"$2$3 <code class="ml-1.5 text-xs font-mono font-semibold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded whitespace-nowrap" aria-label="Sección del notebook" data-nb-ref-badge>&sect;&thinsp;${sectionRef}</code>$4`,
        );
    }
    return result;
}
...
export function CaseContentRenderer({
    ...
    const md = useMemo<MarkdownMap>(() => {
        ...
        return {
            ...
            m3Content: injectNotebookRefs(rich(content.m3Content)),
    ...
    useEffect(() => {
        const frame = requestAnimationFrame(() => {
            ...
            headings.forEach((element) => {
                // Clone before reading textContent so that injected badge nodes
                // (data-nb-ref-badge) are excluded from the SectionRail label.
